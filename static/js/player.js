...
function renderArrival(state, me, privateData) {
  ...
  arrivalPanel.innerHTML = `
    ...
    ${state.pending_action?.player_id === playerId ? `<div class="callout sapphire">대기 행동: ${escapeHtml(state.pending_action.type)}</div>` : ""}
  `;
}
...
function renderRequests(state, privateData) {
  ...
  requestPanel.innerHTML = `<h2>거래 및 승인 요청</h2>${requests.map((offer) => `<div class="request-card">${requestDetails(state, offer)}</div>`).join("") || "<p>관련 요청이 없습니다.</p>"}`;
}
...
function renderActionState() {
  ...
  Array.from(buildingType.options).forEach((option) => { option.disabled = available.length > 0 && !available.includes(option.value); });
}
